...
// 아이유 노래 리스트 
    ...
    // 그니까, id 가 넘어오면 id 기반, 
    // title 이 넘어오면 title 기반 으로 코드가 진행될껀데 이를 먼저 판단하고 싶어. 
    int rId=0;
    ...
    // 로직상, 둘 중 하나는 null  id정보가 null 이 아니면, id 기반으로 간다. 아니면 title 기반
    boolean idBase = (request.getParameter("id")!=null);
			...
			<div class="artist d-flex border border-success p-3">
				...
				<% if(idBase){
					for(Map i : musicList) { 	
						if(rId == (int)(i.get("id"))) {%>
				<div>
					<img width="150" src="<%= i.get("thumbnail")%>">
				</div>
				<div class="ml-3">
					<h2><%= i.get("title") %></h2>
					<div class="text-success"><%= i.get("singer") %></div>
					<div class="smtext mt-3">
						앨범 		<%= i.get("album") %><br>
						재생시간	<%= i.get("time") %><br>
						작곡가	<%= i.get("composer") %><br>
						작사가 	<%= i.get("lyricist") %>
					</div>
				</div>
				<% } 
